...
const string help3 = "Press X to use bomb. You can carry 2 bombs at a time.";
const string help4 = "This game may not be completable by you. No need to be frustrated.";
const string help5 = "Do not panic. Sometimes calmly dodging is the key.";
...
const string help8 = "In arcade mode: Press P to pause. Arcade still in Beta.";
...
#define PATTERN_TWINWAVE 1
#define PATTERN_ROTATE 2
...
void Spawner2D::spawnObject() {
	switch(spawntype) {
		...
		case SQUARE_SPECIAL4 : {
			...
			Ptt.add(PatternObj(spawner->getX(), spawner->getY(), num, PATTERN_TWINWAVE));
			Ptt[0].setSpeed(2);
			Ptt[0].setDirection(spawner->getAngle());
			for (int i = 0; i < num; i++) {
		...
		case SQUARE_ROTATER : {
			Ptt.add(PatternObj(spawner->getX(), spawner->getY(), 16, PATTERN_ROTATE));
			Ptt[0].setDirection(270);
			for (int i = 0; i < 16; i++) {
				Sq.add(SquareObj(spawner->getX() + 100*cos(convertToRadian(i*360/16)), spawner->getY()  - 100*sin(convertToRadian(i*360/16)), 40, 3));
				Sq[0].setDirection(i*360/16);
				Sq[0].setSpeed(2.5);
				Ptt[0].add(&Sq[0]);
			}
		} break;
...
void createLevel() {
	...
	switch (gamemode) {
		case ID_LEVEL_A : {
			...
			Spw.add(Spawner2D(new SquareObj(-150, -150, 180, 999), TYPE_SQUARE, 999, 280, NULL_SHOT));
		...
		case ID_SHOOTER2: {
			Spw.add(Spawner2D(new TriangleObj(WINDOW_WIDTH/4, -180, 150, 130, 10, 1), TYPE_TRIANGLE, 1, 40, QUAD_ANGLESHOT1));
			Spw.add(Spawner2D(new TriangleObj(WINDOW_WIDTH*2/4, -180, 150, 130, 10, 1), TYPE_TRIANGLE, 1, 40, QUAD_ANGLESHOT1));
			Spw.add(Spawner2D(new TriangleObj(WINDOW_WIDTH*3/4, -180, 150, 130, 10, 1), TYPE_TRIANGLE, 1, 40, QUAD_ANGLESHOT1));
			Spw.setBegin();
		...
		case ID_ZONE8 : {
			Spw.add(Spawner2D(new SquareObj(WINDOW_WIDTH/2, 10, 100, 25, 1), TYPE_SQUARE, 1, 150, SQUARE_ROTATER));
			Spw[0].getObject()->setSpeed(1);
			Spw[0].getObject()->setDirection(270);
		...
		case ID_RINGSHOOTER : {
			...
			Spw[0].getObject()->getColorData()[1] = 0.3;
			worldn = 1;
		...
		case ID_DANMAKU1 : {
			...
			Spw.add(Spawner2D(new SquareObj(WINDOW_WIDTH/2, -180, 150, 999, 1), TYPE_SQUARE, 0.02, 400, SQUARE_SPECIAL1));
			Spw[0].getObject()->setSpeed(6);
			Spw[0].getObject()->setDirection(270);
...
void bomb() {
	...
	Sq.add(SquareObj(WINDOW_WIDTH/2, WINDOW_HEIGHT/2, 2000, 0.1, 1));
	Sq[0].getColorData()[1] = 0.0;
	Sq[0].getColorData()[2] = 0.0;
}
...
void drawObjects() {
	...
	while (i < Ptt.size()) {
		for (int j = 0; j < Ptt.getIValue().getObjNum(); j++) {
			if (Ptt.getIValue().getObjData()[j] != NULL)
				if (Ptt.getIValue().getObjData()[j]->isDead())
					Ptt.getIValue().getObjData()[j] = NULL;
		}
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (!paused()) {
			...
			if (abs(newypos - ypos) + abs(newxpos - xpos) < 10) {
				xpos = newxpos;
				ypos = newypos;
			...
			case ID_LEVEL_A: {
				if (!paused() && !gameover && Spw.isEmpty()) {
					...
					if (!paused()) {
						...
						if (Spw[0].getHealth() <= 120 && Spw[0].getHealth() > 120 - SHOT_DAMAGE) {
